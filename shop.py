...
def readCustomerCSV():
    ...
    if filenameSelection  == 2:
        # read csv file as a list of lists
        with open('customerOverOrder.csv', 'r') as read_obj:
        # pass the file object to reader() to get the reader object
            csv_reader = reader(read_obj)
        # Pass reader object to list() to get a list of lists
            list_of_rows = list(csv_reader)
...
def receipt(dict,cost,cash,budget):
    ...
    if(budget > cost):
        for x in dict:
            productCost = float(dict[str(x)][0])
            productQuantity = int(dict[str(x)][1])
            print(x,", Cost: €" ,"{:.2f}".format(productCost),", Quantity:",productQuantity)
...
def calRemainingStock(n,stockDic,custOrderedQuan):
    ...
    if updatedStock <= 0:
        ...
        print("302", updatedStock)
        print("303", currentStock)
        return currentStock
    else:
        updatedStock = float(currentStock) - float(custOrderedQuan)
        return updatedStock
...
def receiptl(l,dic,stockDic,cash,budget):
    print("********************")
    print("Receipt")
    ...
    if ( budget > tcost):
        for x in range(len(l)):
            ...
            productCost = l[x][0]
            productQuantity = dic[rDL[x]]
            print("Product:",l[x][0],", Cost: ",productCost, "Quanitiy:",productQuantity)
            prod = l[x][0]
    ...
    # get the cost
            cash += tcost
            print("Total cost €","{:.2f}".format(tcost))
            #€","{:.2f}".format(tcost)
            print("*********************")
    ...
    while(run):
        ...
        try:
            if customerType == 1:
                ...
                print("The shop cash is €", "{:.2f}".format(shopcash))
        except:
